...
export default function HotelRegisteration() {
  ...
  const [averageFoodRate, setAverageFoodRate] = useState(0);
  const [reservationCharges, setReservationCharges] = useState(0);
  ...
  const validateInputs = () => {
    ...
    // Validate
    if (!lettersRegex.test(restname)) {
      toast({
        description: "Please enter a valid restaurant name",
        status: "error",
        position: "top",
        duration: 2000,
      });
  ...
  const handleRegister = async (event) => {
    ...
    try {
      ...
      await axios.post("http://127.0.0.1:8000/registerRestaurant", {
        userId: userId,
        name: restname,
        businessName,
      ...
      toast({
        description:
          "Registration submitted successfully! You will be notified when it gets approved by admin.",
        status: "success",
        duration: 5000,
        position: "top",
      });
