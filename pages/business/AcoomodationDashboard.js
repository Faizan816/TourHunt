...
export default function HotelDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const accResponse = await axios.post(
        "http://127.0.0.1:8000/getAccommodation",
        { userId }
      );
      console.log("this is response", accResponse.data);
      const accommodation = accResponse.data || null;
      let expandArray = [];
  ...
  const handleDelete = async (id) => {
    const response = await axios.post(
      "http://127.0.0.1:8000/deleteAccommodation",
      { id }
    );
  ...
  return (
    <Layout>
      <DeleteDialog
        ...
        description="Are you sure you want to delete this Accommodation details?"
      ...
      <Box w={"80%"}>
        <Box w={"100%"} pt={"3%"} pl={"2%"} pb={"1%"} pr={"2%"}>
          <HStack spacing={4} justify="space-between">
            ...
            <HStack
              onClick={() => router.push("../business/Accommodation")}
              _hover={{ opacity: 0.5 }}
              cursor="pointer"
              gap="4px"
            >
              <Image src="/images/plus.png" h="10px" />
              <Text fontSize="small">Register a New Accommodatione</Text>
            </HStack>
          </HStack>
          {accommodation && accommodation.length === 0 ? (
            <Center color="white" h="100vh">
              <b>Kindly register your Accommodation service</b>
            </Center>
          ) : (
            accommodation &&
                ...
                <HStack w={"100%"} fontSize="sm" justifyContent={"flex-end"}>
                  <HStack>
                    <Box
                      ...
                      onClick={() =>
                        router.push({
                          pathname: "../business/EditAccommodation",
                          query: { id: acc._id },
                        })
                      }
                    >
                    ...
                    <AccordionItem>
                      ...
                      <AccordionPanel>
                        ...
                        >
                          ...
                          <GridItem>
                            <Text fontWeight="bold">Amenitites:</Text>
                          </GridItem>
                          <GridItem>
                            <Text>{acc.amenities}</Text>
                          </GridItem>
                          <GridItem>
                            <Text fontWeight="bold">
                              Room Expense Per Person:
                            </Text>
                          </GridItem>
                          <GridItem>
                            <Text>Rs.{acc.hotelRoomExpensePerPerson}</Text>
                          </GridItem>
                          <GridItem>
