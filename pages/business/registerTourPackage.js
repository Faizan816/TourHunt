...
export default function RegisterTourPackage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/registerBusinessTourPackage",
        {
          userId,
          name,
          contact,
      ...
      toast({
        ...
        description: "Your registration will be verified and approved by admin",
        duration: 3000,
        isClosable: true,
        status: "success",
      });
      console.log("Tour package registered successfully:", response.data);
      router.push("../business/tourPackage");
    } catch (error) {
      console.error("Failed to register tour package:", error);
      toast({
    ...
    <Layout>
      <VStack w="100%" p="2%" pt="40px">
        <Box w="100%" borderRadius="10px" p="20px" bg="#143E56">
          <Heading size="sm">Tour Package Registration</Heading>
          <VStack mt="2%">
