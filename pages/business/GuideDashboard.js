...
export default function HotelDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const guResponse = await axios.post("http://127.0.0.1:8000/getGuide", {
        userId,
      });
      const guide = guResponse.data || null;
      setGuide(guide);
      let expandArray = [];
  ...
  const handleDelete = async (id) => {
    const response = await axios.post("http://127.0.0.1:8000/deleteGuide", {
      id,
    });
  ...
  return (
    <Layout>
      <DeleteDialog
        ...
        description="Are you sure you want to delete this Guide Detail?"
      ...
      <Box w={"80%"}>
        <Box w={"100%"} pt={"3%"} pl={"2%"} pb={"1%"} pr={"2%"}>
          <HStack spacing={4} justify="space-between">
            ...
            <HStack
              onClick={() => router.push("../business/Guide")}
              _hover={{ opacity: 0.5 }}
              cursor="pointer"
              gap="4px"
            >
              <Image src="/images/plus.png" h="10px" />
              <Text fontSize="small">Register a New Guide</Text>
            </HStack>
          </HStack>
          {guide && guide.length === 0 ? (
            <Center color="white" h="100vh">
              <b>Kindly register your Guide service</b>
            </Center>
          ) : (
            guide &&
            guide.map((gu, i) => (
                    ...
                    <AccordionItem>
                      ...
                      <AccordionPanel>
                        ...
                        >
                          ...
                          <GridItem>
                            <Text>{gu.location}</Text>
                          </GridItem>
                          <GridItem>
                            <Text fontWeight="bold">Years of experience:</Text>
                          </GridItem>
                          <GridItem>
                            <Text>{gu.years_of_experience}</Text>
                          </GridItem>
                          <GridItem>
                            <Text fontWeight="bold">Specialities:</Text>
                          </GridItem>
                          <GridItem>
                            <Text>{gu.specialization}</Text>
                          </GridItem>
                          <GridItem>
                            <Text fontWeight="bold">Language(s):</Text>
                          </GridItem>
                          <GridItem>
                            <Text>{gu.languages}</Text>
                          </GridItem>
                          <GridItem>
